...
#_{:clj-kondo/ignore [:clojure-lsp/unused-public-var]}
(defn validate-registry [registry] (sim-de-registry/validate registry))

#_{:clj-kondo/ignore [:clojure-lsp/unused-public-var]}
(defn validate-model-data
  "Validate `model-data`."
  [model-data]
  (sim-de-model-data/validate-humanize model-data))
...
(defn scheduler
  ...
  * `stopping-causes`, see [[automaton-simulation-de.impl.stopping-definition.registry]] for possible values."
  ([{::sim-de-model/keys [initial-snapshot]
     :as model}]
...
#_{:clj-kondo/ignore [:clojure-lsp/unused-public-var]}
(defn validate-response [response] (sim-de-response/validate response))
