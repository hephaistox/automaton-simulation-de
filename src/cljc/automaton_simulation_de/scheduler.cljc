...
(defn handler
  ...
  * `request`"
  ...
  (let [{:keys [::sim-de-request/stop
    ...
    (cond
      (seq stop) (sim-de-response/add-current-event response current-event)
      (nil? event-execution)
      (sim-de-event-registry/add-registry-stop response current-event)
...
(defn iterate
  ...
  * `snapshot` snapshot that will be used to iterate
  ...
  (let [{:keys [::sim-de-snapshot/future-events]} snapshot
        ...
        event-execution (sim-de-event-registry/get registry current-event)
        request
