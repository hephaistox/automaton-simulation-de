  ...
  sim-de-computation/Computation
    ...
    (scheduler-response [_ stopping-criterias it]
      (let [it (or it 1)
            ...
            stopping-registry (::sim-engine/stopping (::sim-engine/registry
                                                      (:model @storage)))
            stopping-criterias (->> stopping-criterias
                                    (map (partial
                                          sim-de-criteria/api-data-to-entity
                                          stopping-registry))
                                    (filter some?)
                                    (mapv sim-de-criteria/out-of-model)
                                    (mapv (fn [stp-criteria]
                                            (assoc-in stp-criteria
                                             [::sim-engine/params :chunk-user?]
                                             true))))
            stopping-criterias-valid?
            (->> stopping-criterias
        ...
        (if (not stopping-criterias-valid?)
          (sim-de-comp-response/build
           :internal-error
           #:automaton-simulation-de.simulation-engine{:stopping-criterias
                                                       stopping-criterias})
          (do
                ...
                (loop [{::sim-engine/keys [stopping-causes snapshot]
                  ...
                  (cond
                    ...
                    (do (swap! storage assoc-in
                        ...
                        (load-chunk!
                         storage
                         (+ chunk-size (get snapshot ::sim-engine/iteration 0)))
                        (sim-de-comp-response/build
                         :success
    ...
    (stopping-criterias [_]
      (::sim-engine/stopping-criterias (:model @storage))))
