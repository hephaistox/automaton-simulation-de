...
(defn- endless-sim?
  [stopping-causes]
  (some #(= :endless-sim (get-in % [::sim-engine/stopping-criteria ::sim-engine/params :reason]))
        stopping-causes))
...
(defn- stopping-criteria-match-cause?
  ...
  (if (empty? stopping-criterias)
    true
    (some (fn [stop-cause]
            (some #(and (= (get-in stop-cause [::sim-engine/stopping-criteria ::sim-engine/params])
                           (second %))
                        (= (get-in stop-cause
  ...
  sim-de-computation/Computation
    (scheduler-response [_ stopping-criterias it]
      (let [it (or it 1)
            ...
            endless-stopping-criteria [::sim-engine/iteration-nth {::sim-engine/n endless-sim-catch
                                                                   :reason :endless-sim}]
            it-snapshot (if (<= it 1)
        ...
        (loop [snapshot it-snapshot
          ...
          (let [{::sim-engine/keys [snapshot stopping-causes]
                ...
                (sim-engine/scheduler model
                                      []
                                      (conj stopping-criterias endless-stopping-criteria)
                                      snapshot)]
            (cond
              (endless-sim? stopping-causes) (sim-de-comp-response/build :timeout resp)
              (stopping-criteria-match-cause? stopping-criterias stopping-causes)
              (sim-de-comp-response/build :success resp)
              (= endless-catch-internal-counter max-it) (sim-de-comp-response/build :internal-error
                                                                                    resp)
              (sim-de-computation/not-next? stopping-causes) (sim-de-comp-response/build :no-next
                                                                                         resp)
