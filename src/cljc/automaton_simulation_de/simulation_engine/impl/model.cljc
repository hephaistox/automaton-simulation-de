(ns automaton-simulation-de.simulation-engine.impl.model
  ...
  * `stopping-criterias` list of `stopping-criteria` that defines the end of the model."
  (:require
   ...
   [automaton-simulation-de.simulation-engine.impl.middleware.registry :as
                                                                       sim-de-middleware-registry]
   [automaton-simulation-de.simulation-engine.impl.middlewares         :as sim-de-middlewares]
...
(defn build
  ...
  Retuns the `model`'s."
  [{::sim-engine/keys [initial-bucket middlewares ordering stopping-criterias future-events]
    :or {initial-bucket 0}
    :as model-data}
   registry]
  (let [updated-middlewares (->> middlewares
                                 (mapv (partial sim-de-middleware-registry/data-to-fn
