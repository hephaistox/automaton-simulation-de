...
(defn- composed-pred-schema
  ...
  ([one-param?]
   [:schema {:registry {::composed-pred-schema
                        [:cat
                         pred-name
                         (if one-param?
                           [:or
                            [:cat :keyword [:* :any]]
                            [:schema [:ref ::composed-pred-schema]]]
                           [:+
                            [:or
                             [:cat :keyword [:* :any]]
                             [:schema [:ref ::composed-pred-schema]]]])]}}
...
(def composed-predicates-lang-reg
  ...
  {::sim-pred/not
   {:doc "Returns true if predicate evaluates to false, false otherwise"
    :pred-fn not-fn
   ...
   ::sim-pred/and {:doc "Returns true if ALL of them evaluate to true"
                   :pred-fn and-fn
                   :validation-fn
                   #(core-schema/validate-data (composed-pred-schema) %)}
