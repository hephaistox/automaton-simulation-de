...
(def model-data-stub-1
  #::sut{:initial-event-type :IN
         :ordering [[::sut/field ::sut/date]]
...
(deftest validate-middleware-data-test
  ...
  (is (= nil
         (sut/validate-middleware-data [:supp-middlewares-insert
  ...
  (is (map? (sut/validate-middleware-data :int {}))
      "Non vector middlewares are detected.")
...
(deftest validate-stopping-criteria-data-test
  ...
  (is (= nil (sut/validate-stopping-criteria-data [] {}))
      "No stopping criteria is possible.")
...
(deftest extract-snapshot-test
  (is
   (= nil
      (sut/extract-snapshot (sut/build-model model-data-stub-1 sut/registries)))
...
(defn- extract-stopping-causes
  ...
  (->> response
       ...
       (mapv #(get-in
               %
               [::sut/stopping-criteria ::sut/stopping-definition ::sut/id]))
       sort
       vec))

(defn- extract-snapshot-id
  [response]
  (get-in response [::sut/snapshot ::sut/id]))

(defn- extract-snapshot-date
  [response]
  (get-in response [::sut/snapshot ::sut/date]))
...
(deftest assembly-test
  ...
  (is
   (= [[::sut/no-future-events] 2 0 nil]
      (-> (sut/build-model model-data-stub-1
                           (-> (sut/registries)
                               (assoc-in [::sut/event :IN]
  ...
  (is
   (= [[::sut/iteration-nth ::sut/iteration-nth ::sut/no-future-events] 2 0 nil]
      (-> (sut/build-model model-data-stub-1
                           (-> (sut/registries)
                               (assoc-in [::sut/event :IN]
                                         (sim-de-common/init-events [] 0))))
          (sut/scheduler [::sut/response-validation :request-validation]
                         [[::sut/iteration-nth {::sut/n 2}]
                          [::sut/iteration-nth {::sut/n 2}]])
          causes-and-snapshot-id-date-and-validation))
   "When more than one stopping-criteria exists, they are all returned.")
  (is
   (= [[::sut/iteration-nth] 2 0 nil]
      (-> (sut/build-model model-data-stub-1
                           (-> (sut/registries)
                               (assoc-in [::sut/event :IN]
  ...
  (is (= [[::sut/execution-not-found] 3 0 nil]
         (-> (sut/build-model model-data-stub-1
                              (-> (sut/registries)
                                  (assoc-in [::sut/event :IN]
                                            (sim-de-common/init-events
  ...
  (is (= [[::sut/execution-not-found] 3 1 nil]
         (-> (sut/build-model model-data-stub-1
                              (-> (sut/registries)
                                  (assoc-in [::sut/event :IN]
                                            (sim-de-common/init-events
