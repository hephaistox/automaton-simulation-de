...
(comment
  ...
  (sim-engine/validate-stopping-criteria-data [[::sim-engine/iteration-nth #::sim-engine{:n 30}]]
                                              {})
  (tap> (sim-engine/scheduler model [[:state-rendering state-rendering]] []))
  (def full-run (sim-engine/scheduler model [[:state-rendering state-rendering]] []))
  (def s1
    (sim-engine/scheduler model
                          [:tap-request :tap-resonse]
                          [[::sim-engine/iteration-nth ::sim-engine {:n 30}]]))
  (def s2
    (->> s1
